...
d3.csv("https://keisuke168.github.io/InfoVis2022/FinalTask/netflix.csv")
    .then(data => {
        ...
        let hist = new Histogram(preprocess(data))
        hist.update(2015);
        ...
        inputSliderBarElement.addEventListener('change', function() {
          ...
          document.getElementById('year2').innerHTML = inputSliderBarElement.value;
          wd.update(inputSliderBarElement.value)
          hist.update(inputSliderBarElement.value)
      });
    });
...
function preprocess(data){
  ...
  data.forEach(d => {
    ...
    if(!isNaN(d['IMDb Score']) && d['IMDb Score']!=''){
      score[year].push(d['IMDb Score'])
    }
  });
  return score;
}
